...
def get_timestamp_from_uuid():
    generation_time = dt.datetime.utcfromtimestamp((0x01b21dd213814000) * 100 / 1e9)
...
def uuid_to_object_id(uuid_object):
    ...
    timestamp_of_uuid = hex(get_timestamp_from_uuid())
    ...
    result = (timestamp_of_uuid + fourth + res)
    return ObjectId(result[:len(result) - 1])
